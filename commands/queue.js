...
module.exports = {
  ...
  const permissions = message.channel.permissionsFor(message.client.user);
    if (!permissions.has(["MANAGE_MESSAGES", "ADD_REACTIONS"]))
      return sendError("lol i need reaction and manage message perms",message.channel);

    const queue = message.client.queue.get(message.guild.id);
    if (!queue) return sendError("bruh theres nothing playing rn.",message.channel)
...
function generateQueueEmbed(message, queue) {
  ...
  for (let i = 0; i < queue.length; i += 10) {
    ...
    const embed = new MessageEmbed()
     .setAuthor("Server Songs Queue", "https://media.discordapp.net/attachments/804454352713941036/869581748378402866/nithibot.gif")
