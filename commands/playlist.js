	...
	run: async function (client, message, args) {
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			try {
				...
				if (!playlist) return sendError("bruh playlist not found", message.channel)
			...
			try {
				...
				if (searched.playlists.length === 0) return sendError("bruh i couldnt find the song", message.channel)
				...
				let thing = new MessageEmbed()
					.setAuthor("Playlist has been added to queue", "https://media.discordapp.net/attachments/804454352713941036/869581748378402866/nithibot.gif")
...
async	function play(guild, song) {
  ...
  let afk = JSON.parse(fs.readFileSync("./afk.json", "utf8"));
    ...
    if (!song){
      if (!online.afk) {
        sendError("lmao im leaving the vc. say this command for 24/7 music lol: `!afk`", message.channel)
...
let thing = new MessageEmbed()
				.setAuthor("gg music started!", "https://media.discordapp.net/attachments/804454352713941036/869581748378402866/nithibot.gif")
				.setThumbnail(song.img)
				.setColor("BLUE")
				.addField("Name", song.title, true)
				.addField("Duration", song.duration, true)
